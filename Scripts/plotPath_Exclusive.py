...
parser = argparse.ArgumentParser(description='A script to plot the path exclusive timing histogram for more than one path or more than one test. Default is to make one plot per path (i.e. test same path for one or more input files). To make it do more than one path on the same plot, pass the --pathTest flag. Makes an output file called PathExclusive_<PathName>.png for default mode and PathExclusive_PathComparison.png for --pathTest flag. Usage: python validation_plot.py --inputfiles INPUTFILES --runs RUNNUMBERS --processes PROCESSNAMES --nthreads NTHREADS --outdir OUTDIR --log')
...
def plotPathComparison(f,run,process,paths):
    ...
    tFile = TFile(f)
    for p in paths:
        hName = "DQMData/Run %s/HLT/Run summary/TimerService/Running %s processes/process %s/Paths/%s_exclusive" % (run,nthreads[0],process,p)
    ...
    output = open(outdir+"pathexclusive.txt","w")
    ...
    leg.Draw("same")
    filename = 'PathExclusive_PathComparison'
    if args.log:
        filename+='_log'
    c.Print(outdir+filename+'.png')
...
#make canvas to save plots to
    ...
    Thists=[]
    while j<len(Tfiles):
        dirname="DQMData/Run %s/HLT/Run summary/TimerService/Running %s processes/process %s/Paths/%s_exclusive" % (runs[j],nthreads[0],processes[j],path)
        print dirname
